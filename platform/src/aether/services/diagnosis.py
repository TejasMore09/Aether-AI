...
def diagnose_approval(tenant_id: uuid.UUID, approval_id: uuid.UUID) -> str:
    ...
    with tenant_session(tenant_id) as db:
        ...
        ] or ["- (no stored observations; decision came from explicit values)"]

        user_prompt = (
            f"Domain: {approval.domain}\n"
            f"Pending decision: {approval.action} | risk {approval.risk_level} | "
            f"estimated loss ${approval.expected_loss_usd:,.2f}/day\n"
            f"Engine reasoning: {approval.reason}\n\n"
            f"Policy thresholds: performance floor {params.perf_threshold}, "
            f"drift threshold {params.drift_threshold}, "
            f"action cost ${params.retrain_cost_usd}\n\n"
            f"Recent observations (newest first):\n" + "\n".join(obs_lines)
        )
